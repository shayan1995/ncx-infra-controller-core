...
while [[ $i -lt $MAX_RETRY ]]; do
  ...
  kubectl exec -ti nico-pg-cluster-0  -n postgres -- /usr/bin/psql -U postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid)
FROM pg_stat_activity
WHERE datname = 'nico_system_nico'
  AND pid <> pg_backend_pid();"

  kubectl exec -ti nico-pg-cluster-0  -n postgres -- /usr/bin/psql -U postgres -c "DROP DATABASE nico_system_nico;"
  if [ $? -eq 0 ]; then
...
kubectl exec -ti nico-pg-cluster-0  -n postgres -- /usr/bin/psql -U postgres -c 'CREATE DATABASE nico_system_nico with owner "nico-system.nico";'
