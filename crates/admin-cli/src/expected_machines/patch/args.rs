...
impl Args {
    pub fn validate(&self) -> Result<(), NicoCliError> {
        ...
        {
            return Err(NicoCliError::GenericError("One of the following options must be specified: bmc-user-name and bmc-password or chassis-serial-number or fallback-dpu-serial-number or bmc-ip-address or dpu-mode".to_string()));
        }
        if self
            .fallback_dpu_serial_numbers
            .as_ref()
