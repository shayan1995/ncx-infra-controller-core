...
//use rpc::nico::NvlPartitionSearchFilter;
...
async fn test_create_instance_with_nvl_config(pool: sqlx::PgPool) {
    ...
    let mut nvl_config = rpc::nico::InstanceNvLinkConfig {
        gpu_configs: gpus
            .iter()
            .filter_map(|gpu| {
                gpu.platform_info.as_ref().map(|platform_info| {
                    rpc::nico::InstanceNvLinkGpuConfig {
    ...
    let request_all = tonic::Request::new(rpc::nico::NvLinkPartitionSearchFilter {
        name: None,
    ...
    env.api
        .delete_nv_link_logical_partition(tonic::Request::new(
            rpc::nico::NvLinkLogicalPartitionDeletionRequest {
    ...
    let request_partitions = tonic::Request::new(rpc::nico::NvLinkLogicalPartitionsByIdsRequest {
...
async fn test_with_multiple_nv_link_logical_partitions(pool: sqlx::PgPool) {
    ...
    let nvl_config = rpc::nico::InstanceNvLinkConfig {
        gpu_configs: gpus
            .iter()
            .filter_map(|gpu| {
                gpu.platform_info.as_ref().map(|platform_info| {
...
async fn test_create_instances_with_nvl_configs_same_logical_partition_different_domains(
    ...
    let mut nvl_config1 = rpc::nico::InstanceNvLinkConfig {
        gpu_configs: gpus1
            .iter()
            .filter_map(|gpu| {
                gpu.platform_info.as_ref().map(|platform_info| {
    ...
    let mut nvl_config2 = rpc::nico::InstanceNvLinkConfig {
        gpu_configs: gpus2
            .iter()
            .filter_map(|gpu| {
                gpu.platform_info.as_ref().map(|platform_info| {
...
async fn test_update_instance_with_nvl_config(pool: sqlx::PgPool) {
    ...
    let new_nvl_config = rpc::nico::InstanceNvLinkConfig {
        gpu_configs: gpus
            .iter()
            .filter_map(|gpu| {
                gpu.platform_info.as_ref().map(|platform_info| {
...
async fn test_create_instance_add_to_existing_partition(pool: sqlx::PgPool) {
    ...
    let nvl_config2 = rpc::nico::InstanceNvLinkConfig {
        gpu_configs: gpus2
            .iter()
            .filter_map(|gpu| {
                gpu.platform_info.as_ref().map(|platform_info| {
...
async fn test_update_nvlink_config_nmxm_existing_partitions_with_nmxc_simulator(
    ...
    assert!(
        ...
        "expected a NICo-managed NMX-C partition with four GPUs after instance update, got: {nmxc_partitions:?}"
    );
}
