...
async fn test_ib_fabric_monitor(pool: sqlx::PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        env.test_meter
            .formatted_metric("nico_ib_monitor_fabrics_count")
            .unwrap(),
        "1"
    );
    assert_eq!(
    ...
    assert_eq!(
        env.test_meter
            .formatted_metric("nico_ib_monitor_ufm_version_count")
            .unwrap(),
        r#"{fabric="default",version="mock_ufm_1.0"} 1"#
    );
    assert_eq!(
        env.test_meter
            .formatted_metric("nico_ib_monitor_fabric_error_count"),
        None
    );
    // The default partition is found
    assert_eq!(
        env.test_meter
            .formatted_metric("nico_ib_monitor_ufm_partitions_count")
            .unwrap(),
        r#"{fabric="default"} 1"#
    );
    assert_eq!(
        env.test_meter
            .formatted_metric("nico_ib_monitor_iteration_latency_milliseconds_count")
            .unwrap(),
        r#"1"#
    );
    ...
    assert_eq!(
        env.test_meter
            .formatted_metric("nico_ib_monitor_insecure_fabric_configuration_count")
            .unwrap(),
        r#"{fabric="default"} 0"#
    );
    assert_eq!(
        env.test_meter
            .formatted_metric("nico_ib_monitor_allow_insecure_fabric_configuration_count")
            .unwrap(),
        r#"{fabric="default"} 0"#
    );
