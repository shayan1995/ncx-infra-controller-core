...
pub async fn handle_delete_measurement_system_profile(
    ...
    let profile: Option<MeasurementSystemProfile> = match req.selector {
        ...
        None => {
            return Err(
                NicoError::InvalidArgument("profile selector is required".to_string()).into(),
            );
        }
    };

    let system_profile = profile.ok_or(NicoError::NotFoundError {
        kind: "profile",
