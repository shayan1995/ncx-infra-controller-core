...
use nico_machine_controller::config::power_manager::default_power_options;
...
use nico_rack_controller::config::{RackValidationConfig, RmsConfig};
use nico_site_explorer::config::SiteExplorerConfig;
use nico_state_controller_common::config::StateControllerConfig;
use nico_utils::config::{as_duration, as_std_duration};
use chrono::Duration;
...
pub struct NicoConfig {
    ...
    /// continue to rely on nico-dhcp's option 67 for the URL.
    ///
    ...
    /// surface a runtime `NotSupported` error; nico-dhcp option 67 is the
    /// fallback URL source.
    #[serde(default)]
...
impl From<&FnnRoutingProfileConfig> for rpc::nico::RoutingProfile {
    fn from(profile: &FnnRoutingProfileConfig) -> Self {
        Self {
            ...
            accepted_leaks_from_underlay: profile
                ...
                .iter()
                .map(|entry| rpc::nico::PrefixFilterPolicyEntry {
...
mod tests {
    ...
    fn legacy_force_dpu_nic_mode_in_toml_still_parses() {
        let _config: NicoConfig = Figment::new()
            .merge(Toml::file(format!("{TEST_DATA_DIR}/min_config.toml")))
            .merge(Toml::string(
