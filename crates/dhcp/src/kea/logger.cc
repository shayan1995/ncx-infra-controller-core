...
isc::log::Logger ffi_logger("nico-rust");
...
extern "C" {
	...
	void kea_log_generic_debug(int level, char* message) {
		LOG_DEBUG(ffi_logger, level, isc::log::LOG_NICO_GENERIC).arg(message);
	}
	void kea_log_generic_info(char* message) {
		LOG_INFO(ffi_logger, isc::log::LOG_NICO_GENERIC).arg(message);
	}
	void kea_log_generic_warn(char* message) {
		LOG_WARN(ffi_logger, isc::log::LOG_NICO_GENERIC).arg(message);
	}
	void kea_log_generic_error(char* message) {
		LOG_ERROR(ffi_logger, isc::log::LOG_NICO_GENERIC).arg(message);
	}
}
