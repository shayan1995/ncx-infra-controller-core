...
pub fn reason_to_user_string(p: &rpc::nico::ControllerStateReason) -> Option<String> {
    use rpc::nico::ControllerStateOutcome::*;
    let Ok(outcome) = rpc::nico::ControllerStateOutcome::try_from(p.outcome) else {
        tracing::error!("Invalid rpc::nico::ControllerStateOutcome i32, should be impossible.");
        return None;
    };
    match outcome {
