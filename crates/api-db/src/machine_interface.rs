...
/// either falling into a NICo-managed segment (when there is a match within a managed
...
/// DPU's `nico-dhcp-server`, from the host config generated by
/// `get_managed_host_network_config`.
/// The central Kea + `nico-dhcp` path must not answer for these links. Under that invariant it
...
/// correct for the DPU-side `nico-dhcp-server` admin path, where the generated active host config
